...
class LinkedList:
    ...
    def recursive_sum(self):
        ...
        def node_sum(node):
            if node is None:
                return 0
            return node.data + node_sum(node.next)
    ...
    def recursive_reverse(self):
        ...
        def helper(prev, current):
            ...
            current.next = prev # set current node 'next' to previous node
            return helper(current, current_next) # advance the node recursion
        
        return helper(prev=None, current=self.head)
